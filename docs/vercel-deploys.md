...
`npm run lint`, on every push) is the authoritative test gate. The Vercel `build`
...
_Verification note: this line changes only a Markdown file (removed after the ignoreCommand skip is confirmed)._
