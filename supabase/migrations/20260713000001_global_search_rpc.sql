-- Applies the global_search RPC that src/components/dashboard/GlobalSearchModal.tsx
-- has been calling since the command-palette redesign. The function was authored
-- in supabase_global_search.sql (repo root) but that file was never turned into a
-- migration, so `global_search` has never actually existed in the database — every
-- call has been failing silently (caught, logged, results left empty), which is why
-- typing in the search palette never produced live results. Logic copied verbatim,
-- not rewritten.
