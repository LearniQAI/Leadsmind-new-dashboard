-- Email Sequences S2: saving a workflow used to DELETE every step and re-INSERT them, one
-- statement at a time with no transaction. workflow_executions.current_step_id is
-- ON DELETE SET NULL, so every in-flight contact lost its position and the executor then
-- marked them 'completed' -- a contact mid-sequence silently lost all remaining emails. A
-- failure part-way through also left a half-built workflow.
--
-- save_workflow_graph does the whole save in ONE transaction and edits in place:
--   * steps are matched to their existing rows (by id when the caller is id-aware, else by
--     position+type) and UPDATED, so a running contact's current_step_id stays valid and the
--     step-log history is kept;
--   * new steps are inserted; steps that no longer exist are deleted, and any RUNNING
--     execution sitting on one is explicitly moved to the next surviving step (its wait /
--     hold timers are cleared, so it proceeds at the next sweep) or, if nothing follows,
--     completed with a recorded reason -- never silently dropped;
--   * edges are rewritten in the same transaction.
-- SECURITY INVOKER: runs as the caller under RLS (check_workspace_access), and re-checks the
-- workspace itself. No new SECURITY DEFINER surface.
--
-- p_fields: {name, description?, trigger_type, trigger_config?, is_active}
