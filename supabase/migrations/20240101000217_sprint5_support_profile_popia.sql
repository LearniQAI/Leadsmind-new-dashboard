...
DROP POLICY IF EXISTS "service role full access email_change_requests" ON public.email_change_requests;
...
-- Guarded: these tables must already exist (created by an earlier support-ticket migration).
-- If they don't exist yet, this block is skipped rather than failing the whole migration.
DO $$
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'ticket_attachments') THEN
