-- Phase 34 Fixed Final Fix: Universal Integration & Intelligence Bridge
...
-- 3. Automation Trigger Registry (Ensuring it exists)
...
-- 4. Integration Logic: Auto-Mark Completion (Using 'appointments' table)
CREATE OR REPLACE FUNCTION fn_integ_handle_appointment_outcome()
RETURNS TRIGGER AS $$
BEGIN
    -- If appointment is marked as no-show
...
-- 5. Trigger for Appointment Outcomes
DROP TRIGGER IF EXISTS tr_integ_appointment_outcome ON appointments;
CREATE TRIGGER tr_integ_appointment_outcome
AFTER UPDATE ON appointments
FOR EACH ROW EXECUTE FUNCTION fn_integ_handle_appointment_outcome();
