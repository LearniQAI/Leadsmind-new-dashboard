...
function getClassString(props: any): string {
  const classes = cn(
    props.className,
    formatPseudoClasses(props.customClasses, props.hoverClasses, props.focusClasses)
  );
...
export function renderCraftToHtml(
          ...
          try {
            const res = await fetch('/api/builder/submit', {
              ...
              body: JSON.stringify({ 
                ...
                formId: e.target.id || 'default_form'
              })
            });
            ...
            if (result.success) {
              if (config.onSuccess === 'redirect' && config.redirectUrl) {
                ...
                showToast(config.successMessage || 'Submission successful!', 'success');
                e.target.reset();
              }
            } else {
  ...
  if (fullPage) {
    const title = siteConfig?.title || 'Published Page';
    const description = siteConfig?.description || '';
    const ogImage = siteConfig?.ogImage || '';
    const headingFont = siteConfig?.headingFont || 'Space Grotesk';
    const bodyFont = siteConfig?.bodyFont || 'DM Sans';
    const primaryColor = siteConfig?.primaryColor || '#6c47ff';
    const secondaryColor = siteConfig?.secondaryColor || '#3b82f6';
    const backgroundColor = siteConfig?.backgroundColor || '#ffffff';

    const fontImport = `@import url('https://fonts.googleapis.com/css2?family=${encodeURIComponent(headingFont)}:wght@400;500;700;900&family=${encodeURIComponent(bodyFont)}:wght@400;500;700&display=swap');`;
    ...
    `;
    return minifyHtml(fullPageHtml);
  }
...
function renderNode(node: CraftNode, allNodes: CraftContent): string {
  ...
  const classesStr = getClassString(props);
  ...
  switch (name) {
    ...
    case 'Spacer':
      return `<div ${classesStr} style="width: 100%; height: ${props.height}px;"></div>`;
    ...
    case 'Divider':
      ...
      return `<div ${classesStr} style="width: 100%; padding-top: ${props.paddingTop}px; padding-bottom: ${props.paddingBottom}px;"><div style="height: ${props.weight}px; background-color: ${props.color}; width: ${props.width}; ${alignStyle}"></div></div>`;
    ...
    case 'Heading':
      ...
      return `<${headingTag} ${classesStr} style="${weightStyle} text-align: ${props.textAlign || 'left'}; color: ${props.color || 'inherit'}; ${props.fontSize ? `font-size: ${props.fontSize}px;` : ''} ${mapPropsToStyle(props)}">${props.text || ''}</${headingTag}>`;

    case 'Paragraph':
      return `<p ${classesStr} style="font-size: ${props.fontSize || 16}px; text-align: ${props.textAlign || 'left'}; color: ${props.color || 'inherit'}; ${props.lineHeight === 'tight' ? 'line-height: 1.25;' : props.lineHeight === 'loose' ? 'line-height: 2;' : 'line-height: 1.625;'} ${mapPropsToStyle(props)}">${props.text || ''}</p>`;
    ...
    case 'Image':
      ...
      return `<div ${classesStr} style="width: ${props.width || '100%'}; height: ${isCircle ? (props.width || '100%') : (props.height || 'auto')}; display: flex; justify-content: center; ${mapPropsToStyle(props)}"><img src="${props.src || ''}" alt="${props.alt || ''}" style="width: 100%; height: 100%; ${shapeStyles} display: block;" /></div>`;
    ...
    case 'Video':
      ...
      if (props.provider === 'custom') {
        return `<div ${classesStr} style="width: 100%; position: relative; padding-top: 56.25%; overflow: hidden; border-radius: ${props.borderRadius || 0}px; ${mapPropsToStyle(props)}"><video src="${props.url}" ${props.autoPlay ? 'autoplay' : ''} ${props.controls ? 'controls' : ''} ${props.loop ? 'loop' : ''} ${props.muted ? 'muted' : ''} style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; object-fit: cover;" /></div>`;
      }
      return `<div ${classesStr} style="width: 100%; position: relative; padding-top: 56.25%; overflow: hidden; border-radius: ${props.borderRadius || 0}px; ${mapPropsToStyle(props)}"><iframe src="${embedUrl}" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; border: none;" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe></div>`;

    case 'Icon':
      return `<div ${classesStr} style="display: inline-flex; color: ${props.color || 'inherit'}; ${mapPropsToStyle(props)}"><svg width="${props.size || 24}" height="${props.size || 24}" viewBox="0 0 24 24" fill="${props.fill ? 'currentColor' : 'none'}" stroke="currentColor" stroke-width="${props.strokeWidth || 2}" stroke-linecap="round" stroke-linejoin="round" class="lucide"><circle cx="12" cy="12" r="10"/><path d="M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3"/><path d="M12 17h.01"/></svg></div>`;

    case 'Text':
      return `<div ${classesStr} style="font-size: ${props.fontSize}px; text-align: ${props.textAlign || 'left'}; color: ${props.color || 'inherit'}; ${mapPropsToStyle(props)}">${props.text || ''}</div>`;
    ...
    case 'Form':
      const formFields = (props.fields || []).map((f: any) => {
        ...
        if (f.type === 'textarea') {
          ...
          const opts = (f.options || ['Option 1']).map((o: string) => `<div style="display: flex; align-items: center; gap: 8px;"><input type="radio" name="${fieldName}" value="${o}" /><label style="font-size: 14px; color: ${props.labelColor || '#333'};">${o}</label></div>`).join('');
      ...
      return `
        <div ${classesStr} style="padding: ${props.padding || 32}px; border-radius: ${props.borderRadius || 12}px; background: ${props.backgroundColor || '#fff'}; ${mapPropsToStyle(props)}">
          <form id="${props.formId || 'form_' + Math.random().toString(36).substr(2, 9)}" style="display: flex; flex-direction: column;" data-config="${formConfig}">
            ${formFields}
    ...
    case 'PricingTable':
      ...
      `).join('');
      return `<div ${classesStr} style="display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 24px; padding: 32px;">${plansHtml}</div>`;
    ...
    case 'FAQ':
      const faqsHtml = (props.items || []).map((item: any) => `
        <div style="border: 1px solid ${props.borderColor || '#eee'}; border-radius: ${props.borderRadius || 12}px; padding: 24px; margin-bottom: ${props.gap || 16}px; background: ${props.itemBg || '#fff'}; box-shadow: 0 1px 2px rgba(0,0,0,0.05);">
          <div style="font-weight: bold; font-size: 18px; margin-bottom: 12px; color: ${props.questionColor || '#111'};">${item.question}</div>
          <div style="color: ${props.answerColor || '#666'}; font-size: 16px; line-height: 1.6; opacity: 0.9;">${item.answer}</div>
        </div>
      `).join('');
      return `<div ${classesStr} style="max-width: 900px; margin: 0 auto; padding: ${props.padding || 48}px 24px;">${faqsHtml}</div>`;

    case 'Testimonial':
    case 'UserTestimonial':
      return `
        <div ${classesStr} style="padding: ${props.padding || 48}px; background: ${props.backgroundColor || '#fff'}; border-radius: ${props.borderRadius || 24}px; text-align: ${props.textAlign || 'center'}; border: 1px solid rgba(0,0,0,0.05);">
          ...
          <blockquote style="font-size: 18px; color: ${props.textColor || '#111'}; margin-bottom: 24px; line-height: 1.6;">"${props.quote}"</blockquote>
    ...
    case 'StarRating':
      ...
      return `
        <div ${classesStr} style="text-align: ${props.alignment || 'center'}; padding: 16px 0;">
    ...
    case 'LogoStrip':
      const logos = (props.logos || []).map((l: any) => `
        <img src="${l.src}" alt="${l.alt}" style="height: ${props.height || 32}px; filter: ${props.grayscale ? 'grayscale(100%)' : 'none'}; opacity: ${(props.opacity || 50) / 100}; object-fit: contain;" />
      `).join('');
      return `
        <div ${classesStr} style="background: ${props.backgroundColor || 'transparent'}; padding: ${props.padding || 48}px 0;">
    ...
    case 'Hero':
      return `
        <div ${classesStr} style="min-height: ${props.minHeight || 80}vh; background-color: ${props.backgroundColor || '#fff'}; background-image: ${props.layout === 'background' ? `url(${props.backgroundImage})` : 'none'}; background-size: cover; background-position: center; padding: ${props.padding || 80}px 24px; position: relative; display: flex; align-items: center; justify-content: center;">
          ...
          <div style="position: relative; z-index: 2; max-width: 1200px; width: 100%; display: flex; flex-direction: ${props.layout === 'split' ? 'row' : 'column'}; align-items: center; gap: ${props.gap || 40}px; text-align: ${props.layout === 'split' ? 'left' : 'center'};">
            ...
            ${props.layout === 'split' ? `<div style="flex: 1;"><img src="${props.backgroundImage || 'https://images.unsplash.com/photo-1460925895917-afdab827c52f?q=80&w=2426'}" style="width: 100%; border-radius: 24px; box-shadow: 0 25px 50px -12px rgb(0 0 0 / 0.25);" /></div>` : ''}
          </div>
        </div>
      `;
    ...
    case 'Navbar':
      return `
        <div ${classesStr} style="background: ${props.backgroundColor || '#0f172a'}; color: ${props.textColor || '#fff'}; padding: ${props.padding || 16}px 24px; position: ${props.sticky ? 'sticky' : 'relative'}; top: 0; z-index: 1000;">
    ...
    case 'Footer':
      ...
      `).join('');
      return `
        <div ${classesStr} style="background: ${props.backgroundColor || '#0f172a'}; color: ${props.textColor || '#fff'}; padding: ${props.padding || 80}px 24px;">
    ...
    case 'BlogFeed':
      const items = [...Array(props.count || 3)].map((_, i) => `
        <div style="background: ${props.cardBg || '#fff'}; border-radius: ${props.borderRadius || 24}px; overflow: hidden; border: 1px solid rgba(0,0,0,0.05);">
          <div style="height: 200px; background: #eee; overflow: hidden;"><img src="https://images.unsplash.com/photo-1460925895917-afdab827c52f?q=80&w=2426" style="width: 100%; height: 100%; object-fit: cover;" /></div>
