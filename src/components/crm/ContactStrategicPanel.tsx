  ...
  return (
    <div className="w-[240px] border-l border-white/5 flex flex-col bg-[#080f28] h-full shrink-0 overflow-y-auto no-scrollbar">
      ...
      <div className="p-6 flex flex-col items-center text-center border-b border-white/5">
        ...
        <p className="text-[12px] text-[#4a5a82] font-dm-sans mb-4">{contact.email || 'No email'}</p>
        ...
        <div className="flex flex-wrap justify-center gap-1.5">
          ...
          {contact.tags && contact.tags.slice(0, 1).map(tag => (
            <div key={tag} className="bg-[#10b981]/15 text-[#10b981] text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-widest font-dm-sans">
              {tag}
            </div>
          ))}
        </div>
      ...
      <div className="p-6 space-y-6">
        <div>
          ...
          <div className="flex items-center gap-3 bg-white/[0.03] border border-white/5 rounded-[12px] p-3">
            <i className="fa-solid fa-earth-americas text-[#3b82f6] text-[12px]"></i>
            <span className="text-[13px] font-semibold text-[#eef2ff] font-dm-sans capitalize">{contact.source || 'Direct Entry'}</span>
          </div>
        </div>
