              ...
              <div className="max-w-[850px] mx-auto bg-white border border-dash-border text-slate-900 p-12 md:p-16 rounded-2xl shadow-lg printable-area min-h-[1000px] relative overflow-hidden">
                ...
                <div className="flex justify-between items-start mb-16 relative z-10">
                  <div>
                    ...
                    <div className="text-[11px] font-bold uppercase tracking-[0.2em] text-slate-400 space-y-2 leading-relaxed">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-16 relative z-10">
                  <div className="bg-slate-50 p-8 rounded-2xl border border-slate-100">
                    <span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-4 flex items-center gap-2">
                    ...
                    <p className="text-sm font-semibold text-slate-500 mb-5">{selectedInvoice.contact?.email}</p>
                  ...
                  <table className="w-full min-w-[500px]">
                    <thead>
                      <tr className="border-b border-slate-100 bg-slate-50">
                        <th className="py-5 px-6 text-[9px] font-black uppercase tracking-widest text-slate-400 text-left">Description</th>
                        <th className="py-5 px-6 text-[9px] font-black uppercase tracking-widest text-slate-400 text-right">Qty</th>
                        <th className="py-5 px-6 text-[9px] font-black uppercase tracking-widest text-slate-400 text-right">Rate</th>
                        <th className="py-5 px-6 text-[9px] font-black uppercase tracking-widest text-slate-400 text-right">Amount</th>
                      </tr>
                    </thead>
                      ...
                      {(selectedInvoice.items || []).map((item: any, idx: number) => {
                        ...
                        return (
                          <tr key={idx} className="hover:bg-white transition-colors">
                            ...
                            <td className="py-6 px-6 text-right text-sm text-slate-500 font-bold">{quantity}</td>
                            <td className="py-6 px-6 text-right text-sm text-slate-500 font-bold">${rate.toLocaleString()}</td>
                ...
                <div className="flex justify-end pt-2 relative z-10">
                  <div className="w-[380px] space-y-4">
                    <div className="flex justify-between items-center text-slate-500 text-xs font-bold px-6">
                      <span className="uppercase tracking-widest text-[10px]">Subtotal</span>
                      <span>${(Number(selectedInvoice.subtotal || selectedInvoice.total_amount) || 0).toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between items-center text-slate-500 text-xs font-bold pb-5 border-b border-slate-100 px-6">
                    ...
                    <div className="flex justify-between items-center pt-2 mt-4 bg-primary/5 border border-primary/20 rounded-2xl p-8 relative overflow-hidden">
                      <div className="flex flex-col gap-1.5 relative z-10">
                        ...
                        <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">USD</span>
                      </div>
