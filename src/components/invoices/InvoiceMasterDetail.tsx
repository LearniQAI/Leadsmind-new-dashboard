                  ...
                  <div className="card__wrapper !p-4 !mb-0 flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-xl no-print">
                     <div className="flex items-center gap-4">
                        ...
                        <div className="overflow-hidden">
                           <h3 className="card__title !text-lg !mb-0 italic truncate">Invoice Ledger</h3>
                     ...
                     <div className="flex gap-2 w-full md:w-auto">
                        <Button
                           ...
                           className="btn-primary h-10 px-6 rounded-xl italic uppercase text-[10px] gap-2"
                        >
                        ...
                        <Button 
                           onClick={handlePrint}
                           variant="outline" 
                           className="btn btn-outline-theme-border h-10 px-6 rounded-xl italic uppercase text-[10px] gap-2"
                        >
                  ...
                  <div className="flex-1 overflow-y-auto scrollbar-none">
                     <div className="card__wrapper !p-8 md:!p-16 !mb-0 text-white border-white/5 shadow-2xl relative overflow-hidden group min-h-full printable-area">
                        ...
                        <div className="flex flex-col md:flex-row justify-between items-start gap-8 mb-16 md:mb-20 relative z-10">
                           <div>
                              <div className="h-10 w-40 bg-white text-black rounded-xl flex items-center justify-center font-black italic mb-6 tracking-tighter text-sm uppercase">LEADSMIND</div>
                              <div className="text-[10px] font-black uppercase tracking-[0.2em] text-white/40 space-y-2 italic">
                           ...
                           <div className="text-left md:text-right w-full md:w-auto">
                              <h1 className="text-5xl md:text-7xl font-black italic uppercase tracking-tighter text-white mb-2 leading-none text-balance">Invoice</h1>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-16 md:mb-20 relative z-10">
                           ...
                           <div className="grid grid-cols-2 gap-4 pt-0 md:pt-6">
                              <div>
                                 ...
                                 <span className="text-sm font-black italic text-white whitespace-nowrap">{format(new Date(selectedInvoice.created_at), 'dd MMM yyyy')}</span>
                              </div>
                              <div>
                                 <span className="text-[8px] font-black uppercase tracking-[0.2em] text-white/30 block mb-2">Settlement Due</span>
                                 <span className="text-sm font-black italic text-rose-500 whitespace-nowrap">{selectedInvoice.due_date ? format(new Date(selectedInvoice.due_date), 'dd MMM y') : 'Immediate'}</span>
                              </div>
                           </div>
                        </div>
                              ...
                              <tbody className="divide-y divide-white/5">
                                 {selectedInvoice.items?.map((item: any, idx: number) => (
                                    <tr key={idx} className="group/row">
                                       <td className="py-8 font-black italic uppercase text-sm text-white/90 group-hover/row:text-primary transition-colors">{item.description}</td>
                                       ...
                                       <td className="py-8 text-right font-black italic text-white text-xl">${(Number(item.quantity || 1) * Number(item.unit_amount || 0)).toLocaleString()}</td>
                                    </tr>
                                 ))}
                                 {(!selectedInvoice.items || selectedInvoice.items.length === 0) && (
                                    <tr className="group/row">
                                       <td className="py-10 font-black italic uppercase text-sm text-white/90 group-hover/row:text-primary transition-colors">Standard Consultation Services</td>
                                       ...
                                       <td className="py-10 text-right font-black italic text-white text-2xl">${Number(selectedInvoice.total_amount).toLocaleString()}</td>
                                    </tr>
                                 )}
                              </tbody>
                           </table>
                        ...
                        <div className="flex justify-end pt-12 border-t-2 border-white/10 relative z-10">
                           <div className="w-full md:w-80 space-y-6">
                              <div className="flex justify-between items-center text-white/40 italic">
                              ...
                              <div className="flex justify-between items-center text-white/40 italic pb-6 border-b border-white/5">
                              ...
                              <div className="flex justify-between items-center pt-6">
                                 ...
                                 <span className="text-4xl md:text-6xl font-black italic tracking-tighter text-white">${Number(selectedInvoice.total_amount).toLocaleString()}</span>
                              </div>
                           </div>
                        </div>
