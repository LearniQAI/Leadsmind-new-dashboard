 ...
 return (
  ...
  >
   <div className={`${fullWidth ? 'w-full' : 'max-w-7xl mx-auto'} w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-${Math.min(4, columnsCount + 1 + (activeNewsletterToggle ? 1 : 0))} gap-x-12 gap-y-16`}>
    ...
    {columns.map((col: { title: string, links: { label: string, href: string }[] }, i: number) => (
     <div key={i} className="flex flex-col gap-6">
      <h4
       className="uppercase tracking-[0.2em] mb-2 whitespace-nowrap"
       style={{
        color: accentColor,
     ...
     <div className="flex flex-col gap-6 lg:min-w-[300px]">
      ...
      <div className="flex flex-col sm:flex-row gap-2">
       <input
        ...
        className="bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-xs flex-1 outline-none focus:border-white/20 transition-colors"
       />
       <button
        className="px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all hover:brightness-110 active:scale-95"
   ...
   {(showSocial && socialLinks.length > 0) && (
    <div className={`${fullWidth ? 'w-full' : 'max-w-7xl mx-auto'} w-full mt-16 flex flex-col sm:flex-row gap-6 pt-12 border-t border-white/5 items-center`}>
     <span className="text-[10px] font-black uppercase tracking-widest opacity-40">Connect with us</span>
     <div className="flex gap-4">
      {socialLinks.map((social: { platform: string, url: string }, i: number) => (
       <a key={i} href={social.url} className="w-11 h-11 rounded-2xl flex items-center justify-center bg-white/5 hover:bg-white/10 transition-all border border-white/5">
