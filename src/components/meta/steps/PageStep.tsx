          ...
          <div className="absolute z-20 mt-1.5 w-full bg-dash-surface border border-dash-border rounded-lg shadow-xl overflow-hidden py-1 animate-in fade-in slide-in-from-top-1 duration-150">
            {options.map((opt) => (
              <div
                ...
                className="px-4 py-2.5 hover:bg-dash-bg cursor-pointer transition-colors flex flex-col gap-0.5"
              >
                <span className="text-[13px] font-medium text-dash-text">{opt.name}</span>
                <span className="text-[11px] text-dash-textMuted">{opt.slug}</span>
              </div>
            ))}
          </div>
        )}
