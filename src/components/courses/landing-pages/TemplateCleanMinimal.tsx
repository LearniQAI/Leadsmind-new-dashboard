...
import { BookOpen, Play, Lock, Eye, Star, Plus, Minus, Check } from 'lucide-react';
...
export default function TemplateCleanMinimal({ course, modules, lessons, previewData }: TemplateProps) {
  ...
  const activeTemplate = previewData?.template || course?.landing_page_settings?.template || 'clean_minimal';
    ...
    <div className="min-h-screen bg-n900 text-t1 font-sans antialiased py-12 px-6">
      <div className="max-w-4xl mx-auto space-y-16">
        ...
        {isSectionVisible('hero') && (
          <section className="text-center space-y-6 pb-8 border-b border-white/5">
            <h1 className="text-4xl md:text-5xl font-display font-bold text-t1 tracking-tight max-w-2xl mx-auto">
              {pageTitle}
            </h1>
            <p className="text-base text-t2 max-w-xl mx-auto leading-relaxed">
              {tagline}
            </p>
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-2">
              <button 
                onClick={handleEnroll}
                className="w-full sm:w-auto bg-accent hover:bg-accent/90 text-white font-bold text-xs uppercase tracking-widest px-8 py-3.5 rounded-xl shadow-lg transition-all"
              >
                Enroll in Course
              </button>
            </div>
        ...
        {isSectionVisible('outcomes') && outcomes.length > 0 && (
          <section className="space-y-6">
            <h2 className="text-xl font-display font-semibold text-t1 border-l-2 border-accent pl-3">
              What you will learn
            </h2>
                  ...
                  <div key={mod.id} className="bg-n800 border border-white/5 rounded-xl overflow-hidden transition-all">
                    ...
                    >
                      <div className="space-y-1">
                        <span className="text-[10px] font-bold uppercase tracking-wider text-accent2">Module</span>
        ...
        {isSectionVisible('instructor') && (
          <section className="bg-n800 border border-white/5 p-6 rounded-2xl flex flex-col md:flex-row items-center md:items-start gap-6">
            ...
            <div className="space-y-2 text-center md:text-left">
              <span className="text-[10px] font-bold uppercase tracking-widest text-accent2">Your Instructor</span>
        ...
        {isSectionVisible('pricing') && (
          <section className="bg-gradient-to-br from-n800 to-n900 border border-white/10 p-8 rounded-2xl text-center space-y-4">
            <span className="text-[10px] font-bold uppercase tracking-widest text-amber">Enrollment Access</span>
            ...
            <p className="text-xs text-t2 max-w-md mx-auto">
              Get lifetime access to all core modules, expandable lessons, direct Q&A, and certification nodes.
            </p>
            <button 
              onClick={handleEnroll}
              className="bg-accent hover:bg-accent/90 text-white font-bold text-xs uppercase tracking-widest px-8 py-3.5 rounded-xl shadow-lg transition-all mt-2"
            >
              Get Instant Access
            </button>
          </section>
