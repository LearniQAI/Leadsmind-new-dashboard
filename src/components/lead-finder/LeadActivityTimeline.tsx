...
import { addLeadNote } from '@/app/actions/lead-workspace';
import { Clock, Send, FileText, User, RefreshCw, Tag, Link as LinkIcon, Loader2 } from 'lucide-react';
...
export function LeadActivityTimeline({ leadId, activities, notes }: { leadId: string, activities: Activity[], notes: Note[] }) {
  ...
  const getIcon = (type: string) => {
    switch (type) {
      ...
      case 'tag_added':
      case 'tag_removed': return <Tag size={14} className="text-amber-400" />;
  ...
  return (
    <div className="bg-n800 border border-white/10 rounded-2xl p-6 flex flex-col h-[600px]">
      <h3 className="text-lg font-space font-bold text-white mb-6 flex items-center gap-2">
        <Clock className="text-accent" /> Activity Timeline
      </h3>
      ...
      <div className="flex-1 overflow-y-auto pr-4 space-y-6 relative custom-scrollbar">
        ...
        {timeline.length === 0 ? (
          <p className="text-t4 text-sm text-center pt-8">No activity recorded yet.</p>
        ) : (
          timeline.map((item, i) => (
      ...
      <form onSubmit={handleAddNote} className="mt-6 pt-6 border-t border-white/10 relative">
        <textarea
          ...
          placeholder="Log a note or manual update..."
