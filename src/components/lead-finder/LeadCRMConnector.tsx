      ...
      {pipelineId ? (
        <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-t3">Active in Pipeline</p>
          </div>
        </div>
      ) : (
        <div className="space-y-3">
          <p className="text-sm text-t3">This lead is currently floating in the workspace. Push to pipeline to manage opportunities.</p>
          <button
            onClick={handlePush}
            disabled={loading}
