...
export default async function LeadWorkspacePage({ params }: { params: { id: string } }) {
  ...
  if (!success || !data || !data.lead) {
    return (
      <Wrapper>
        <div className="p-12 text-center !text-dash-textMuted">Lead not found or unauthorized.</div>
      </Wrapper>
    );
  }
              ...
              <div className="relative z-10">
                <div className="flex flex-wrap items-start justify-between gap-4 mb-6">
                  <div>
                    ...
                    <h1 className="text-3xl md:text-4xl font-black !text-dash-text mb-2">{lead.business_name}</h1>
                    
                    <div className="flex flex-wrap items-center gap-4 text-sm !text-dash-textMuted">
                      {lead.rating > 0 && (
                        <span className="flex items-center gap-1 text-amber-400 font-bold">
                          <Star size={16} className="fill-amber-400" /> {lead.rating} <span className="!text-dash-textMuted font-normal">({lead.review_count} reviews)</span>
                        </span>
                      )}
                      ...
                      {lead.lead_score > 0 && (
                        <span className="flex items-center gap-1.5 text-dash-accent font-bold"><Activity size={16} /> Score: {lead.lead_score}</span>
                      )}
                    </div>
                  </div>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-white border border-dash-border rounded-2xl p-6">
                  <div className="space-y-4">
                    <h3 className="text-xs font-bold !text-dash-textMuted tracking-wider mb-2">Contact Details</h3>
                    {lead.address && (
                      <div className="flex items-start gap-3 !text-dash-textMuted text-sm">
                        <MapPin className="text-dash-accent shrink-0" size={18} />
                    ...
                    {lead.website && (
                      <div className="flex items-center gap-3 !text-dash-textMuted text-sm">
                        <Globe className="text-dash-accent shrink-0" size={18} />
                        <a href={lead.website} target="_blank" rel="noopener noreferrer" className="text-dash-accent hover:underline line-clamp-1">{lead.website}</a>
                      </div>
                    )}
                  </div>

                  <div className="space-y-4">
                    <h3 className="text-xs font-bold !text-dash-textMuted tracking-wider mb-2">Digital Presence</h3>
                    <div className="flex items-center gap-3 !text-dash-textMuted text-sm">
                      <Linkedin className={lead.linkedin_url ? "text-[#0A66C2]" : "!text-dash-textMuted"} size={18} />
                      {lead.linkedin_url ? (
                        <a href={lead.linkedin_url} target="_blank" rel="noopener noreferrer" className="hover:!text-dash-text transition-colors">LinkedIn Profile</a>
                      ) : <span className="!text-dash-textMuted">Not Found</span>}
                    </div>
                    <div className="flex items-center gap-3 !text-dash-textMuted text-sm">
                      <Facebook className={lead.facebook_url ? "text-[#1877F2]" : "!text-dash-textMuted"} size={18} />
                      {lead.facebook_url ? (
                        <a href={lead.facebook_url} target="_blank" rel="noopener noreferrer" className="hover:!text-dash-text transition-colors">Facebook Page</a>
                      ) : <span className="!text-dash-textMuted">Not Found</span>}
                    </div>
                    {lead.description && (
                      <p className="text-xs !text-dash-textMuted italic mt-2 line-clamp-2">"{lead.description}"</p>
                    )}
                  </div>
                </div>
              </div>
