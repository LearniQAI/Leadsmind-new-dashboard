...
export default async function ContactWorkspacePage({ params }: { params: { id: string } }) {
  ...
  if (!success || !data || !data.contact) {
    return (
      <Wrapper>
        <div className="p-12 text-center text-white">Contact not found or unauthorized.</div>
      </Wrapper>
    );
  }
                ...
                <div className="flex flex-wrap items-start justify-between gap-4 mb-6">
                  <div>
                    <div className="flex items-center gap-2 mb-3">
                      <span className="text-xs font-black uppercase tracking-widest text-accent bg-accent/10 px-3 py-1 rounded-sm">
                        {contact.department || 'Contact'}
                      </span>
                      <span className={`flex items-center gap-1 px-3 py-1 rounded-sm text-xs font-bold tracking-widest uppercase border ${
                    ...
                    <h1 className="text-3xl md:text-4xl font-space font-black text-white mb-2">{contact.first_name} {contact.last_name}</h1>
                    <p className="text-lg text-t2 font-semibold mb-4">{contact.title}</p>
                  </div>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {/* Personal Details */}
                  <div className="bg-n900 border border-white/5 rounded-2xl p-6 space-y-4">
                    <h3 className="text-xs font-bold text-t4 uppercase tracking-wider mb-2">Direct Contact</h3>
                    <div className="flex items-center gap-3 text-t2 text-sm">
                      <Mail className="text-accent shrink-0" size={18} />
                      {contact.email ? <span>{contact.email}</span> : <span className="text-t4 italic">Not available</span>}
                    </div>
                    <div className="flex items-center gap-3 text-t2 text-sm">
                      <Phone className="text-accent shrink-0" size={18} />
                      {contact.phone ? <span>{contact.phone}</span> : <span className="text-t4 italic">Not available</span>}
                    </div>
                    <div className="flex items-center gap-3 text-t2 text-sm">
                      <Linkedin className={contact.linkedin_url ? "text-[#0A66C2]" : "text-t4"} size={18} />
                      {contact.linkedin_url ? (
                        <a href={contact.linkedin_url} target="_blank" rel="noopener noreferrer" className="hover:text-white transition-colors">LinkedIn Profile</a>
                      ) : <span className="text-t4 italic">Not available</span>}
                    </div>
                  </div>

                  {/* Company Association */}
                  <div className="bg-n900 border border-white/5 rounded-2xl p-6 space-y-4">
                    <h3 className="text-xs font-bold text-t4 uppercase tracking-wider mb-2">Associated Company</h3>
                    <div className="flex items-start gap-3 text-t2 text-sm">
                      <Building2 className="text-t4 shrink-0 mt-0.5" size={18} />
                      <div>
                        <Link href={`/lead-finder/lead/${contact.result_id}`} className="font-bold text-white hover:text-accent transition-colors">
                          {lead?.business_name}
                        </Link>
                        <p className="text-xs text-t4 mt-1">{lead?.industry || lead?.category}</p>
                      </div>
                    </div>
                    {lead?.website && (
                      <div className="flex items-center gap-3 text-t2 text-sm mt-3">
                        <Globe className="text-t4 shrink-0" size={18} />
                        <a href={lead.website} target="_blank" rel="noopener noreferrer" className="hover:text-white transition-colors line-clamp-1">{lead.website}</a>
                      </div>
                    )}
                  </div>
                </div>
              ...
              <div className="bg-n800 border border-white/10 rounded-3xl p-6">
                ...
                <div className="grid grid-cols-1 gap-3">
                  {insights.map((insight, idx) => (
                    <div key={idx} className="p-4 rounded-xl bg-white/5 border border-white/10 flex items-start gap-3 text-t2">
