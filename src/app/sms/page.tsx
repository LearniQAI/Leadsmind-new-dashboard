...
import { getSmsReadiness } from '@/lib/smsReadiness';
...
export default async function BulkSmsPage() {
  ...
  const [campaignsRes, segmentsRes, readiness] = await Promise.all([listBulkSmsCampaigns(), listSegments(), getSmsReadiness(workspaceId)]);
    ...
    <MetaData pageTitle="Bulk SMS">
      <Wrapper>
        <div className="p-6 max-w-7xl mx-auto font-body min-h-[calc(100vh-80px)]">
          <SmsClient initialCampaigns={campaigns} segments={segments} smsReady={readiness.ready} />
        </div>
      </Wrapper>
    </MetaData>
  );
