...
export default async function PortalDashboard() {
  ...
  return (
    <MetaData pageTitle="Client Dashboard">
      <div className="max-w-6xl mx-auto space-y-12 p-8 md:p-12 bg-dash-bg">
        {/* Header */}
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
          <div>
            <h1 className="text-3xl md:text-4xl font-bold uppercase tracking-tight font-space text-dash-text">
              Portal <span className="text-dash-accent">Overview</span>
            </h1>
        ...
        {!ficaComplete && (
          <div className="bg-amber-50 border border-amber-200 text-amber-800 p-6 rounded-3xl flex flex-col md:flex-row items-center justify-between gap-4 shadow-sm animate-in slide-in-from-top-4 duration-300">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 rounded-2xl bg-amber-100 flex items-center justify-center text-amber-600 shrink-0">
                <AlertTriangle size={24} />
              </div>
              <div>
                <h3 className="text-sm font-bold uppercase tracking-wider text-amber-800 font-space">FICA Onboarding Incomplete</h3>
                <p className="text-xs text-amber-700 mt-1 max-w-lg leading-relaxed font-sans font-medium">
            ...
            <Link
              ...
              className="bg-amber-600 hover:bg-amber-500 text-white font-black px-6 py-3 rounded-xl text-xs uppercase tracking-wider transition-all duration-300 active:scale-95 shrink-0 shadow-md flex items-center gap-1.5"
            >
        ...
        {overdueAlertEnabled && hasOverdueInvoice && (
          <div className="bg-rose-50 border border-rose-200 text-rose-800 p-6 rounded-3xl flex flex-col md:flex-row items-center justify-between gap-4 shadow-sm animate-in slide-in-from-top-4 duration-300">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 rounded-2xl bg-rose-100 flex items-center justify-center text-rose-600 shrink-0">
                <AlertTriangle size={24} />
              </div>
              <div>
                <h3 className="text-sm font-bold uppercase tracking-wider text-rose-800 font-space">Outstanding Balance Overdue</h3>
                <p className="text-xs text-rose-700 mt-1 max-w-lg leading-relaxed font-sans font-medium">
            ...
            <Link
              ...
              className="bg-rose-600 hover:bg-rose-500 text-white font-black px-6 py-3 rounded-xl text-xs uppercase tracking-wider transition-all duration-300 active:scale-95 shrink-0 shadow-md flex items-center gap-1.5"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {/* Card 1: Balance */}
          <div className="bg-white border border-dash-border p-8 rounded-3xl shadow-sm relative overflow-hidden group hover:border-dash-accent/30 transition-all">
            <div className="absolute top-0 left-0 right-0 h-[2px] bg-rose-500" />
            <p className="text-[10px] font-black text-dash-textMuted uppercase tracking-[0.2em] mb-2">Outstanding Due</p>
            <p className="text-3xl font-bold font-space text-rose-600">
              R {outstandingDue.toLocaleString('en-ZA', { minimumFractionDigits: 2 })}
            </p>
            <p className="text-[10px] text-dash-textMuted font-medium mt-1">
          ...
          <div className="bg-white border border-dash-border p-8 rounded-3xl shadow-sm relative overflow-hidden group hover:border-dash-accent/30 transition-all">
            ...
            <p className="text-[10px] font-black text-dash-textMuted uppercase tracking-[0.2em] mb-2">Average Progress</p>
            <p className="text-3xl font-bold font-space text-purple-600">
          ...
          {/* Card 3: Next Session */}
          <div className="bg-white border border-dash-border p-8 rounded-3xl shadow-sm relative overflow-hidden group hover:border-dash-accent/30 transition-all">
            <div className="absolute top-0 left-0 right-0 h-[2px] bg-dash-accent" />
            <p className="text-[10px] font-black text-dash-textMuted uppercase tracking-[0.2em] mb-2">Next Scheduled Meeting</p>
            <p className="text-lg font-bold font-space text-dash-accent truncate mt-1">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-white border border-dash-border rounded-3xl overflow-hidden flex flex-col shadow-sm">
            <div className="p-6 border-b border-dash-border flex items-center justify-between">
              <h3 className="text-sm font-bold uppercase tracking-wider text-dash-text">Recent Invoices</h3>
              <span className="text-[10px] font-black uppercase text-dash-accent">Auto Synced</span>
            </div>
            <div className="overflow-x-auto flex-grow">
              {invoices.length === 0 ? (
                <div className="p-12 text-center text-dash-textMuted text-xs uppercase tracking-wider">
                  No invoices found
                </div>
              ) : (
                  ...
                  <tbody className="divide-y divide-dash-border">
                    {invoices.slice(0, 5).map((inv, i) => (
                      <tr key={i} className="hover:bg-dash-surface transition-all group">
                        ...
                        <td className="px-6 py-5 text-xs text-dash-textMuted font-medium font-mono">
          ...
          {/* Upcoming Bookings */}
          <div className="bg-white border border-dash-border rounded-3xl overflow-hidden shadow-sm flex flex-col">
            <div className="p-6 border-b border-dash-border flex items-center justify-between">
              <h3 className="text-sm font-bold uppercase tracking-wider text-dash-text flex items-center gap-2">
                <Calendar size={16} className="text-dash-accent" /> Upcoming Bookings
              </h3>
              <span className="text-[9px] font-black uppercase px-2 py-0.5 rounded bg-dash-accent/10 text-dash-accent border border-dash-accent/20">
                Live Status
              </span>
            </div>
            ...
            <div className="p-6 flex-1 space-y-4">
              {upcomingBookings.length === 0 ? (
                <div className="py-12 text-center text-dash-textMuted text-xs uppercase tracking-wider">
                  No upcoming sessions scheduled
                </div>
              ) : (
                upcomingBookings.map((appt, i) => (
                  <div key={i} className="p-5 rounded-2xl bg-dash-surface border border-dash-border hover:border-dash-accent/30 transition-all flex items-start justify-between gap-4">
                    <div className="space-y-1">
                      <h4 className="text-sm font-bold text-dash-text">{appt.title}</h4>
                      <p className="text-[11px] text-dash-textMuted font-semibold flex items-center gap-1.5">
                        <span className="w-1.5 h-1.5 rounded-full bg-dash-accent" />
                        {appt.calendar?.name || 'Standard Consultation'}
                      </p>
                      <p className="text-[10px] text-dash-textMuted font-medium mt-1 font-mono">
                    ...
                    {appt.meeting_link && (
                      <a
                        ...
                        className="flex items-center gap-2 px-4 py-2 rounded-xl bg-dash-accent hover:bg-dash-accent/90 text-white text-xs font-bold transition-all shrink-0 uppercase tracking-wider"
                      >
                        Join <Video size={14} />
                      </a>
                    )}
