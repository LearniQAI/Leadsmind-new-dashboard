    ...
    {initialWorkflows.length === 0 ? (
     ...
     initialWorkflows.map(workflow => (
      <div key={workflow.id} className="bg-[#0b0b1a] border border-white/10 rounded-3xl p-6 group hover:border-primary/50 transition-all duration-500 shadow-xl shadow-black/40">
       ...
       <div className="mb-8">
        <h4 className="text-xl font-black text-white uppercase tracking-tighter mb-1 group-hover:text-primary transition-colors">{workflow.name}</h4>
       ...
       <div className="flex items-center justify-between pt-6 border-t border-white/5">
        <div className="flex gap-2">
         <span className="text-[9px] font-black text-white/20 uppercase tracking-widest">Trigger: <span className="text-white/60">{workflow.trigger_type}</span></span>
        </div>
        <div className="flex gap-2">
