...
interface CourseWorkspaceHeaderProps {
  courseTitle: string;
  courseId: string;
  activeTab: "overview" | "modules" | "automations" | "analytics";
  setActiveTab: (tab: "overview" | "modules" | "automations" | "analytics") => void;
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center bg-[#080f28] border border-white/5 rounded-xl p-1 w-fit shrink-0">
        {(["overview", "modules", "automations", "analytics"] as const).map((tab) => (
          <button
            key={tab}
            onClick={() => {
              if (tab === "automations") {
