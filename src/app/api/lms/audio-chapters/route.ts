...
export async function GET(req: NextRequest) {
  try {
    ...
    logger.error({ err }, 'lms.audio_chapters.list.failed');
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { content_block_id, title, start_time_ms, end_time_ms, display_order = 0 } = body;
    if (!content_block_id || !title || start_time_ms == null || end_time_ms == null) {
      return NextResponse.json(
        { error: 'Missing required fields: content_block_id, title, start_time_ms, end_time_ms' },
        { status: 400 }
      );
    }
    ...
    const { data, error } = await adminClient
      .from('audio_chapters')
      .insert({ content_block_id, title, start_time_ms, end_time_ms, display_order })
      .select()
      .single();
    if (error) throw error;
    ...
    logger.error({ err }, 'lms.audio_chapters.create.failed');
    const clientError = toClientError(err);
    return NextResponse.json({ error: clientError.error, code: clientError.code }, { status: clientError.status });
  }
}
