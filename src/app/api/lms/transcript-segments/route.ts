...
export async function GET(req: NextRequest) {
  try {
    ...
    logger.error({ err }, 'lms.transcript_segments.list.failed');
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { content_block_id, speaker_id = null, start_time_ms, end_time_ms, text, sequence = 0 } = body;
    if (!content_block_id || start_time_ms == null || end_time_ms == null || !text) {
      return NextResponse.json(
        { error: 'Missing required fields: content_block_id, start_time_ms, end_time_ms, text' },
        { status: 400 }
      );
    }
    ...
    const { data, error } = await adminClient
      .from('transcript_segments')
      .insert({ content_block_id, speaker_id, start_time_ms, end_time_ms, text, sequence })
      .select()
      .single();
    if (error) throw error;
    ...
    logger.error({ err }, 'lms.transcript_segments.create.failed');
    const clientError = toClientError(err);
    return NextResponse.json({ error: clientError.error, code: clientError.code }, { status: clientError.status });
  }
}
