  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-4xl font-black uppercase tracking-tighter text-white italic leading-none">Marketing <span className="text-primary">Funnels</span></h1>
          <p className="text-white/40 text-sm font-medium mt-2 italic">Design high-performance conversion pathways with neural pathing.</p>
        </div>
        ...
        {initialFunnels.length === 0 ? (
          ...
          initialFunnels.map(funnel => (
            <div key={funnel.id} className="bg-[#0b0b1a] border border-white/10 rounded-3xl p-6 group hover:border-primary/50 transition-all duration-500 shadow-xl shadow-black/40">
              ...
              <div className="mb-8">
                <h4 className="text-xl font-black text-white uppercase italic tracking-tighter mb-1 group-hover:text-primary transition-colors">{funnel.name}</h4>
                <div className="flex items-center gap-2 text-white/30 text-[10px] font-bold tracking-widest">
                  ...
                  <span className="text-primary/70 italic lowercase">/{funnel.subdomain}</span>
                </div>
              </div>
