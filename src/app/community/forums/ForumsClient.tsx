...
import {
  ...
  BookMarked, ChevronRight, X, Loader2, Search, Clock, Bot
} from 'lucide-react';
import { createForumPost, getForumPosts, getPostDetails, addCommentToPost } from '@/app/actions/forum';

// `id` is the value persisted to forum_posts.board (DB CHECK constraint) — do not
// change these strings. Only the descriptions are UI copy.
const BOARDS = [
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      ...
      <div className="flex flex-col gap-4 border-b border-dash-border pb-6 md:flex-row md:items-end md:justify-between">
        <div className="space-y-1.5">
          ...
          <p className="max-w-2xl text-[13px] leading-relaxed text-dash-textMuted">
            ...
            Common questions are answered automatically from our help center.
          </p>
        </div>

        <button
          onClick={() => setIsCreatingPost(true)}
          className="inline-flex h-10 shrink-0 items-center justify-center gap-1.5 rounded-lg bg-sky-500 px-4 text-[13px] font-semibold text-white transition-colors hover:bg-sky-600 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-sky-500/25 [&_svg]:size-4"
        >
          <Plus /> New discussion
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
        ...
        {/* Channel switcher */}
        <nav className="lg:col-span-4 lg:sticky lg:top-6 lg:self-start">
              ...
              return (
                <button
                  ...
                  className={`group flex w-full items-start gap-3 rounded-xl px-3 py-2.5 text-left transition-colors outline-none focus-visible:ring-4 focus-visible:ring-sky-500/20 ${
                    isActive ? 'bg-sky-50 ring-1 ring-inset ring-sky-500/20' : 'hover:bg-dash-surface'
                  }`}
                >
                  <span
          ...
          {loadingPosts ? (
            ...
            <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-dash-border bg-dash-surface/40 px-6 py-16 text-center">
              ...
              <p className="mt-1 max-w-sm text-[12px] leading-relaxed text-dash-textMuted">
                Be the first to start a discussion in {activeBoardMeta ? `“${activeBoardMeta.id}”` : 'this channel'}.
              </p>
              <button
                onClick={() => setIsCreatingPost(true)}
                className="mt-5 inline-flex h-10 items-center justify-center gap-1.5 rounded-lg bg-sky-500 px-4 text-[13px] font-semibold text-white transition-colors hover:bg-sky-600 [&_svg]:size-4"
              >
                <Plus /> New discussion
              </button>
            </div>
                ...
                >
                  <div className="mb-3 flex items-center gap-2.5">
                    ...
                    <div className="min-w-0">
                      <div className="truncate text-[12px] font-semibold text-dash-text">
                  ...
                  <h3 className="text-[14px] font-semibold leading-snug text-dash-text">
                    {post.title}
                  </h3>
                  <p className="mt-1.5 line-clamp-2 text-[13px] leading-relaxed text-dash-textMuted">
                    {post.content}
                  </p>
            ...
            <form onSubmit={handleCreatePost} className="space-y-4 px-6 py-6">
              ...
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  className="h-10 w-full rounded-lg border border-dash-border bg-white px-3 text-[13px] text-dash-text placeholder:text-dash-textMuted outline-none transition-colors focus:border-sky-500 focus:ring-4 focus:ring-sky-500/12"
                />
              </div>
          ...
          <div className="flex h-full w-full max-w-xl flex-col border-l border-dash-border bg-white shadow-xl">
            ...
            <div className="flex items-start justify-between gap-3 border-b border-dash-border px-6 py-4">
              <div className="min-w-0 space-y-1">
                ...
                <h4 className="truncate text-[15px] font-semibold text-dash-text">
            ...
            <div className="flex-1 space-y-5 overflow-y-auto px-6 py-6">
              ...
              <div className="rounded-2xl border border-dash-border bg-dash-surface/50 p-4">
                ...
                <p className="whitespace-pre-wrap text-[13px] leading-relaxed text-dash-text">
            ...
            <form onSubmit={handleAddComment} className="flex gap-2 border-t border-dash-border px-4 py-3">
              <input
                ...
                className="h-10 flex-1 rounded-lg border border-dash-border bg-white px-3 text-[13px] text-dash-text placeholder:text-dash-textMuted outline-none transition-colors focus:border-sky-500 focus:ring-4 focus:ring-sky-500/12"
              />
              <button
                type="submit"
