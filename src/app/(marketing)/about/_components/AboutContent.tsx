          ...
          >
            {team.map((member) => (
              <motion.div
                key={member.name}
                variants={cardItem}
                className="flex flex-col items-center text-center rounded-2xl border border-[#E2E8F0] bg-white p-8 shadow-[0_4px_24px_rgba(15,23,42,0.05)] hover:shadow-[0_16px_44px_rgba(15,23,42,0.1)] hover:-translate-y-1 transition-all duration-300"
              >
                <div className="w-40 h-40 rounded-full overflow-hidden mb-6 ring-1 ring-[#E2E8F0] shadow-[0_8px_20px_rgba(15,23,42,0.08)]">
                  <Image
                    src={member.photo}
                    alt={member.name}
                    width={320}
                ...
                <p className="flex items-center gap-1.5 text-sm !text-[#64748B] mt-1.5">
                  <MapPin className="w-3.5 h-3.5 shrink-0" style={{ color: ROYAL }} />
                  {member.location}
                </p>
              </motion.div>
            ))}
