 ...
 return (
  <div className="space-y-8">
   <div className="flex items-center justify-between">
    <div>
     <h1 className="text-4xl font-black uppercase tracking-tighter text-white leading-none">Neural <span className="text-primary">Commerce</span></h1>
     <p className="text-white/40 text-sm font-medium mt-2">Manage your digital and physical assets with high-frequency precision.</p>
    </div>
    ...
    {initialProducts.length === 0 ? (
     ...
     initialProducts.map(product => (
      <div key={product.id} className="bg-[#0b0b1a] border border-white/10 rounded-3xl p-6 group hover:border-primary/50 transition-all duration-500 shadow-xl shadow-black/40">
       ...
       <div className="mb-8">
        <h4 className="text-xl font-black text-white uppercase tracking-tighter mb-1 group-hover:text-primary transition-colors">{product.name}</h4>
       ...
       <div className="flex items-center justify-between pt-6 border-t border-white/5">
        <div className="flex flex-col">
         <span className="text-[10px] font-black text-white text-xl">${Number(product.price).toLocaleString()}</span>
         <span className="text-[8px] font-black text-white/20 uppercase tracking-widest">{product.currency || 'USD'} per unit</span>
        </div>
