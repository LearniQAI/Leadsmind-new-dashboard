 ...
 return (
  <div className="space-y-8">
   <div className="flex items-center justify-between">
    <div>
     <h1 className="text-4xl font-black uppercase tracking-tighter text-white leading-none">Order <span className="text-primary">Registry</span></h1>
     <p className="text-white/40 text-sm font-medium mt-2">Track and process high-frequency neural transactions.</p>
    </div>
     ...
     <table className="w-full text-left border-collapse">
      ...
      <tbody className="divide-y divide-white/5">
       {initialOrders.length === 0 ? (
        <tr>
         <td colSpan={6} className="px-8 py-20 text-center text-white/20 font-black uppercase tracking-widest">No transaction data available</td>
        </tr>
       ) : (
        initialOrders.map(order => (
         <tr key={order.id} className="hover:bg-primary/5 transition-colors group">
          <td className="px-8 py-5 font-black text-white/60 text-xs">#{order.id.slice(0, 8).toUpperCase()}</td>
          <td className="px-8 py-5">
           <div className="flex items-center gap-3">
            ...
            <div>
             <p className="text-xs font-black text-white uppercase tracking-tight">{order.contact?.first_name} {order.contact?.last_name}</p>
          ...
          <td className="px-8 py-5 font-black text-white text-sm">${Number(order.total).toLocaleString()}</td>
          <td className="px-8 py-5 text-[10px] font-black text-white/20 uppercase tracking-widest">{new Date(order.created_at).toLocaleDateString()}</td>
